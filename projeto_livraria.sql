/*1.Implemente um Banco de Dados chamado “Livraria”. 
Após, implemente as tabelas, conforme o Modelo Relacional dado, 
observando as chaves primárias e as chaves estrangeiras. Todos os campos, de todas as tabelas, 
não podem ser nulos (not null).*/
...
*/
select count(idlivro) as 'Quantidade de livros cadastrados' from livro; -- conta a quantidade de itens inseridos na tabela

/*3. Implemente uma consulta para listar o nome dos clientes cadastrados. A listagem deve ser mostrada em ordem crescente.*/
select nome 'Lista dos nomes dos clientes cadastrados em ordem crescente' from cliente order by nome asc;
...
select editora.nome 'nome da editora', livro.titulo 'Títulos de seus respectivos livros'
from livro 
inner join editora 
on editora.idEditora = livro.idEditora order by nome desc;
...
select editora.nome as 'nome da editora', avg(livro.preco) as 'Média de preço de seus respectivos livros'
from editora 
inner join livro on editora.idEditora = livro.idEditora
group by editora.nome;

/*6. Implemente uma consulta para listar o nome de todos os clientes e a quantidade de títulos de livros comprados pelos mesmos. Para isso, utilize o comando group by.*/


select cliente.nome as 'nome do cliente', count(idPedido) as 'quantidade de títulos de livros comprados pelos mesmos'
from cliente
...
SELECT COUNT(idCliente) FROM pedido;

select nome from cliente; -- mostra o  nome de todos os clientes
select * from cliente;
select * from pedido;
select * from editora;
select * from livro;
...
select count(*), count(idEditora) from livro;
